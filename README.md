...
An AI agent for ParcelPilot support, built on the supplied data pack (policies, SOPs, product docs, customer agreements, and account/order/ticket data).

Supports both user contexts from one app: switch between "Customer" (scoped to one account) and "Staff" (full access + a proactive insights dashboard) in the sidebar.
...
Get a free Gemini key at https://aistudio.google.com/app/apikey (no card required). Default model is gemini-3.6-flash; change GEMINI_MODEL in .env to any Gemini chat model that supports tool calling.
...
"Create a follow-up task for the bulk upload issue on LumenWorks" (watch for the confirmation card — nothing executes until you click Confirm)
